...
class GaussianMixture(torch.nn.Module):
    ...
    def q(self, feat):
        weighted_log_prob = self._estimate_log_prob(feat) + torch.log(self.pi)
        weighted_log_prob = weighted_log_prob.squeeze()
        return torch.sum(weighted_log_prob,axis=-1)
...
if __name__=="__main__":
    ...
    feats = torch.stack(feats, axis=0).unsqueeze(1) # [DatasetSize, Z_dim]
    print(feats.shape)
    labels = torch.randint(0,classes,(ds_size,))

    gmm = GaussianMixture(n_components=classes, n_features=dim, mu_init=None, var_init=None, eps=1.e-6)
    start = time.time()
    gmm.fit(feats)
    end = time.time()
    print(end-start)
